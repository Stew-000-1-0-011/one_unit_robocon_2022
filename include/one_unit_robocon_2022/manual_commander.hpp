...
namespace OneUnitRobocon2022
{
...
#ifdef __cpp_lib_math_constants
                assert_param(max_body_linear_vel, max_body_linear_vel > 0, 2 * std::numbers::pi * 1, "max_body_linear_vel");
#else
...
#ifdef __cpp_lib_math_constants
                assert_param(max_body_angular_vel, max_body_angular_vel > 0, 2 * std::numbers::pi * 0.5, "max_body_angular_vel");
#else
    ...
    private:
        ...
        void case_manual() noexcept
        {
            ...
            one_unit_robocon_2022::TwistPtr cmd_vel_ptr{new one_unit_robocon_2022::Twist};

            auto axes = logicool.get_axes();
            cmd_vel_ptr->linear_x = ros_param_data.max_body_linear_vel * axes[CRSLib::Logicool::Axes::l_stick_UD];
            cmd_vel_ptr->linear_y = ros_param_data.max_body_linear_vel * axes[CRSLib::Logicool::Axes::l_stick_LR];
            cmd_vel_ptr->angular_z = ros_param_data.max_body_angular_vel * axes[CRSLib::Logicool::Axes::r_stick_LR];

            body_twist_pub.publish(cmd_vel_ptr);
